...
def parse_args(notebook=False):
    ...
    parser.add_argument('--seed', type=int, default=999, help='numpy seed')
    parser.add_argument('--factor', type=int, default=100, help='the larger the value, outliers are farther from inliers')
...
def square_sine(level=5, length=1000, freq=0.04, coef=1.5, offset=0.0, noise_amp=0.05):
...
if __name__ == '__main__':
    ...
    - radius: the radius of collective outliers range
    - value = coef * value + offset 
        - value = value + noise_amp * noise # 계수 * 변수(노이즈) + 상수
        - value = np.sin(2 * np.pi * freq * timestamp) # sine 함수
    - option: if 'square': 'level' 'freq' and 'offset' are used to generate square sine wave
              if 'other': 'base' is used to generate outlier shape
    - level: how many sine waves will square_wave synthesis
    '''
    ...
    ####################################
    ## NAME OF THE DATA COMES HERE###
    ###################################
    df_name = f"TS_type0_seed{s}_factor_{f}"
    df_train_name = "train_" + df_name
    df_inference_name = "test_" + df_name
    
    # value = coef * value + offset 
    BEHAVIOR_CONFIG = {'coef': 0.5, "offset": 0.5, 'freq': 3, 'noise_amp': 0.05}
    ...
    # DECIDED NOT TO USE TRAIN DATA AT THE MOMENT (AS OF 02/23)
#     # PLOT TRAIN DATA
...
#     # SAVE TRAIN INFO
#     df.to_csv(f"../result/data/univariate_data/{df_train_name}.csv", index=False)
#     plt.savefig(f"../result/figure/univariate_data/{df_train_name}.jpg")
#     plt.show()
    ...
    univariate_data.point_global_outliers(ratio=0.03, factor=100, radius=6) #0
    # univariate_data.point_contextual_outliers(ratio=0.05, factor=4, radius=6) #1
    # univariate_data.collective_global_outliers(ratio=0.05, radius=6, base=BASE, \
    # coef=1.5, noise_amp=0.05, option='square', level=50, freq=0.04, offset=0.0) #2
    # univariate_data.collective_seasonal_outliers(ratio=0.05, factor=4, radius=6) #3
    # univariate_data.collective_trend_outliers(ratio=0.05, factor=0.5, radius=6) #4
    
    # PLOT INFERENCE DATA
    plt.figure(figsize=(20, 5))
    plt.plot(univariate_data.timestamp, univariate_data.data)
    outlier_idxs = np.where(univariate_data.label == 1)[0]
    plt.plot(univariate_data.timestamp[outlier_idxs], univariate_data.data[outlier_idxs], 'ro')
    ...
    # SAVE INFERENCE INFO
    df.to_csv(f"../result/data/univariate_data/{df_inference_name}.csv", index=False)
    plt.savefig(f"../result/figure/univariate_data/{df_inference_name}.jpg")
    # plt.show()
