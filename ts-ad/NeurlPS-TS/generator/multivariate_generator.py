...
if __name__ == '__main__':
    ...
    df_inference_name = "inference_" + df_name
    
    # 각 COLUMN의 BEHAVIOR
    BEHAVIOR = [sine, cosine, sine, cosine, sine]
    BEHAVIOR_CONFIG = [{'freq': 0.04, 'coef': 1.5, "offset": 0.0, 'noise_amp': 0.15},
                       {'freq': 0.04, 'coef': 2.5, "offset": 0.0, 'noise_amp': 0.05},
                       {'freq': 0.04, 'coef': 1.5, "offset": 0.0, 'noise_amp': 0.05},
                       {'freq': 0.04, 'coef': 2.5, "offset": 2.0, 'noise_amp': 0.10},
                       {'freq': 0.04, 'coef': 1.5, "offset": -2.0, 'noise_amp': 0.05},]

    # LEGNTH는 임의로 1000
    multivariate_data = MultivariateDataGenerator(dim=5, stream_length=500, behavior=BEHAVIOR, behavior_config=BEHAVIOR_CONFIG)
    ...
    # SAVE TRAIN INFO
    df.to_csv(f"./gen_data/multivariate_data/{df_train_name}.csv", index=False)
    plt.savefig(f"./figure/multivariate_data/{df_train_name}.jpg")
    plt.show()
    
    ########################
    ## DEFINE TYPE HERE ####
    ########################    
    # 컬럼 유형 통일 >>> 또는 변경 자유 (m_01011)
    # multivariate_data.point_global_outliers(dim_no=0, ratio=0.05, factor=3.5, radius=5) #0
    # multivariate_data.point_contextual_outliers(dim_no=1, ratio=0.05, factor=2.5, radius=5) #1
    multivariate_data.collective_global_outliers(dim_no=0, ratio=0.05, radius=5, option='square', coef=1.5, noise_amp=0.03, level=20, freq=0.04, offset=0.0) #2 
    multivariate_data.collective_global_outliers(dim_no=1, ratio=0.05, radius=5, option='square', coef=1.5, noise_amp=0.03, level=20, freq=0.04, offset=0.0) #2 
    multivariate_data.collective_seasonal_outliers(dim_no=2, ratio=0.05, factor=3, radius=5) #3
    multivariate_data.collective_seasonal_outliers(dim_no=3, ratio=0.05, factor=3, radius=5) #3
    multivariate_data.collective_trend_outliers(dim_no=4, ratio=0.05, factor=0.5, radius=5) #4
    ...
    ####################################
    ## NAME OF THE FIGURE COMES HERE###
    ###################################
    # SAVE INFERENCE INFO
    df.to_csv(f"./gen_data/multivariate_data/{df_inference_name}.csv", index=False)
    plt.savefig(f"./figure/multivariate_data/{df_inference_name}.jpg")
    plt.show()
